...
// Create an instance of VoyageC class
$voyage = new VoyageC();
...
$offset = ($current_page - 1) * $voyages_per_page;

// Get total number of voyages
$total_voyages = $voyage->getTotalVoyages();
$total_pages = ceil($total_voyages / $voyages_per_page);

// Fetch the paginated list of voyages
$tab = $voyage->getPaginatedVoyages($voyages_per_page, $offset);
...
if ($current_page < 1) {
    ...
    header('Location: liste.php?page=' . $total_pages);
    exit();
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <main class="main-content">
      <header class="admin-header">
        ...
        <div class="admin-actions">
          <button class="btn btn-primary me-2" onclick="openStatsModal()">
  ...
  <div class="modal fade" id="statsModal" tabindex="-1" aria-labelledby="statsModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="statsModalLabel">Statistiques des Voyages</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
  ...
  <script>
    ...
    // Fonction pour ouvrir le modal des statistiques
    function openStatsModal() {
      const modal = new bootstrap.Modal(document.getElementById('statsModal'));
      generateTypesPieChart();
      modal.show();
    }

    // Fonction pour générer le pie chart
    function generateTypesPieChart() {
      // Récupérer tous les types de voyages du tableau
      ...
      rows.forEach(row => {
        const type = row.cells[8].textContent.trim(); // La colonne "Type" est à l'index 8
      ...
      const config = {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: 'Répartition des Voyages par Type',
              font: {
                size: 16
              }
            },
      ...
      if (window.typesPieChart instanceof Chart) {
        window.typesPieChart.destroy();
      }

      // Créer le nouveau graphique
      const ctx = document.getElementById('typesPieChart').getContext('2d');
      window.typesPieChart = new Chart(ctx, config);
    }
  </script>
</body>
</html>
